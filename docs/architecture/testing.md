...
The gbrain test infrastructure: five tiers of test commands, CI vs local divergence, failure-first logging, file taxonomy (`*.test.ts`, `*.slow.test.ts`, `*.serial.test.ts`, `test/e2e/`), test-isolation lint, the `withEnv()` pattern, the canonical PGLite block, the full unit inventory, E2E DB lifecycle, and capturing output rules.

This is the authoritative reference for everything test-related. Anything in `bun run test`, `bun run verify`, `bun run ci:local`, `bun run test:e2e`, or `scripts/run-unit-shard.sh` resolves through here.
